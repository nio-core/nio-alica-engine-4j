...
public abstract class AlicaCommunication {
    ...
    void onSyncTalkReceived(SyncTalk st) {
        ae.getSyncModul().onSyncTalk(st);
    }

    void onSyncReadyReceived(SyncReady sr) {
        ae.getSyncModul().onSyncReady(sr);
    }

    void onAuthorityInfoReceived(AllocationAuthorityInfo aai) {
        ae.getAuth().handleIncomingAuthorityMessage(aai);
    }

    void onPlanTreeInfoReceived(PlanTreeInfo pti) {
        ae.getTeamObserver().handlePlanTreeInfo(pti);
    }

    void onSolverResult(SolverResult sr) {
